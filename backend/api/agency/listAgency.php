...
if (isset($_GET['idAgency'])) {
    ...
    $result = $agency->searchAgencyById($agency);
} else {
     if (isset($_GET['statusAgency'])) {
        $agency->statusAgency = $_GET['statusAgency'];
        if ($agency->statusAgency == 'actif') {
            $agencies = $agency->listActiveAgencies();
        } else {
            $agencies = $agency->listAgenciesByStatus($agency);
        }
    } else {
