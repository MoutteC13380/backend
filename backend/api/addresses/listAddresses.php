...
if (isset($_GET['idAddress'])) {
	...
	if (!is_null($thisBooking['idAddressForth'])) {
		$addressForth = $address->searchAddressById($thisBooking->idAddressForth);
	} else {
		$addressForth = 'Pas d\'adresse Forth';
	}
	if (!is_null($thisBooking['idAddressBack'])) {
		$returnAddress = $address->searchAddressById($thisBooking->idAddressBack);
	} else {
		$addressBack = 'Pas d\'adresse Back';

	}
	$result = [
		'AddressForth' => $addressForth, 
