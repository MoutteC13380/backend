...
if (isset($_GET['idTeammate'])) {
	...
	$result = array();
	array_push($result, $thisTeammate);
} else {
	...
	} else if (isset($_GET['statusTeammate'])) {
		$teammate->statusTeammate = $_GET['statusTeammate'];
		if ($teammate->statusTeammate == 'actif') {
			$teammates = $teammate->searchActiveTeammates($teammate);
		} else {
			$teammates = $teammate->searchTeammatesByStatus($teammate);
		}
	} else {
